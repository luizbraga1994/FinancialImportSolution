...
public sealed class SapIncomingPaymentService : ISapIncomingPaymentService
{
    ...
    public async Task<SapResult> CreateIncomingPaymentAsync(SapSessionContext session, SapIncomingPayment payload, CancellationToken cancellationToken = default)
    {
        try
        {
            var client = _httpClientFactory.CreateClient("SapServiceLayer");
            var request = new HttpRequestMessage(HttpMethod.Post, "IncomingPayments");
            ...
            _logger.LogDebug("SAP IncomingPayment request para {CompanyDb}: {Payload}",
            ...
            {
                var errorMsg = ExtractSapError(rawResponse);
                _logger.LogWarning("SAP IncomingPayment falhou para {CompanyDb}: {Status} - {Error}",
                    session.CompanyDb, response.StatusCode, errorMsg);
                return SapResult.Fail(errorMsg, rawResponse);
            }

            _logger.LogInformation("SAP IncomingPayment criado com sucesso para {CompanyDb}", session.CompanyDb);
            return SapResult.Ok(rawResponse);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Erro ao criar IncomingPayment no SAP para {CompanyDb}", session.CompanyDb);
